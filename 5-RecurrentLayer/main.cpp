...
int main() {
	...
	for (int i = 0; i < 10000000; i++) {
		for (int j = 0; j < x.size(); j++) {
			...
			std::vector<double> dx = p.Backward(mse.Derivative(o, y[j]), 0.001);
		}
		system("clear");

		printf("0 AND 0 = %2.10f,%2.10f\tShould Be: 0,1\n", p.Forward({ 0,0 })[0],p.Forward({ 0,0 })[1]);
		printf("0 AND 1 = %2.10f,%2.10f\tShould Be: 0,1\n", p.Forward({ 0,1 })[0],p.Forward({ 0,1 })[1]);
		printf("1 AND 0 = %2.10f,%2.10f\tShould Be: 0,1\n", p.Forward({ 1,0 })[0],p.Forward({ 1,0 })[1]);
		printf("1 AND 1 = %2.10f,%2.10f\tShould Be: 1,0\n", p.Forward({ 1,1 })[0],p.Forward({ 1,1 })[1]);
	}
