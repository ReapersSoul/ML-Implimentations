...
double linearScale(double value, double min, double max)
{
...
{
	return Color::Lerp(NodeLowColor, NodeHighColor, linearScale(value, -100, 100));
}
static Color WeightColor(double value)
{
	return Color::Lerp(WeightLowColor, WeightHighColor, linearScale(value, -100, 100));
}
static Color BiasColor(double value)
{
	return Color::Lerp(BiasLowColor, BiasHighColor, linearScale(value, -100, 100));
}
...
int main()
{
	...
	while (running)
	{
		...
		std::vector<double> dx = p.Backward(mse.Derivative(o, y[frame% y.size()]), .001);
